...
module FreshBooks
  class Base
    ...
    def self.new_from_xml(xml_root)
      object = self.new
      if xml_root.name == 'client'
        default_currency_code = xml_root.elements['currency_code'].text
        credit_currency_code = xml_root.elements['credit'].attribute('currency').value
        @diff_currency_code = default_currency_code == credit_currency_code ? false : true
        if @diff_currency_code
          xml_root.each_element('credits/credit')  do  |c|
          #puts "-#{c.attribute('currency').value}-#{default_currency_code}-"
            if c.attribute('currency').value == default_currency_code
              @default_currency_credit = c.text
              #puts c.text
            end 
          end
        end
      ...
      self.schema_definition.members.each do |member_name, member_options|
        if member_name == 'credit' and @diff_currency_code
          #puts @default_currency_credit
          object.send("#{member_name}=", @default_currency_credit)
          next
        end
